...
const OutBoxSingleMail = (props) => {
  ...
  const deleteClickHandler = () => {
    fetch(
      `https://mail-box-73bc7-default-rtdb.firebaseio.com/${cleanUserEmail}/sentemails/${endpoint}.json`,
      {
        method: "DELETE",
        headers: {
      ...
      if (res.ok) {
        fetch(
          `https://mail-box-73bc7-default-rtdb.firebaseio.com/${cleanUserEmail}/sentemails.json`
        )
          .then((res) => res.json())
          .then((data) => {
            props.onDelete(data);
