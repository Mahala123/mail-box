...
const SingleMail = (props) => {
  ...
  useEffect(() => {
    fetch(
      `https://mail-box-73bc7-default-rtdb.firebaseio.com/${cleanUserEmail}/inbox/${endpoint}.json`,
      {
        method: "PATCH",
        headers: {
      ...
      if (res.ok) {
        fetch(
          `https://mail-box-73bc7-default-rtdb.firebaseio.com/${cleanUserEmail}/inbox.json`
        )
          .then((res) => res.json())
          .then((data) => {
